...
1. **Third party software**:  Platform operators want pull requests automatically
opened when new versions of cluster add-ons are available so they can see
upcoming changes clearly.
2. **In-house software**: Product development teams want a well defined
deployment pipeline to automatically and safely promote new features and bug
fixes.  For example, from dev to test to staging, then across multiple
