...
term we use for these initial, top level configuration values.  The Platform
Form is the web interface for this model.

#### Role Based Access Control {#RBAC}

Holos platforms configure role based access control to protect services at the
platform layer.  In the k3d platform you're building with this guide, role based
access control is implemented by validating your oidc id token subject claim.
...
Once the form has been submitted, the model needs to be pulled into the local
Git repository.  The `holos` command line operates locally when possible.
...
tools and techniques.  Manage millions of lines of configuration the same way
this guide manages thousands.  This is made possible by the unique way CUE
unifies all configuration into one single model.
