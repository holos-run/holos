...
this guide to protect a service.  We recommend registering an account to see the
how this works.  Registration also enables you to explore the customizable web
form Holos provides to simplify complex configuration.

If you opt-out the guide will use fake authentication headers in place of real
identity tokens.
...
<Tabs groupId="registration">
  <TabItem value="registered" label="Signed In">
    Then, use `holos` to make the rpc call to create the server-side platform
    resource.
...
<Tabs groupId="registration">
  <TabItem value="registered" label="Signed In">
    **Push the Platform Form** to publish it.  Browse to the printed URL to view
    the form.
...
rendering it into plain YAML. Holos does not apply the resulting manifests.
...
⚡ Try not to blink, this is the step where Holos actually builds the platform.  It
usually takes a few seconds.

Rendering the holos reference platform currently results in about half a million
lines of yaml.  Roughly 80,000 lines are produced by this slimmed down k3d
platform.

We mention this because the scale doesn't matter as much as it does with other
tools and techniques.  Manage millions of lines of configuration in the same way
this guide manages thousands.  This is made possible by the unique way CUE
unifies all configuration into one model.
...
tools we're using need to trust these certificates otherwise they won't talk to
each other.  In this section we'll create a local certificate authority that all
of the tools on your system trust.
...
The auth proxy Deployment receives requests from web browsers and provides an
...
https://login.holos.run` to start the auth flow.
...
You should receive a response from the backend httpbin service that echos back
your request.
