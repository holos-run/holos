...
This guide uses httpbin to inspect request headers from the perspective of a backend service, specifically the `x-oidc-id-token` header injected by the authproxy to implement Zero Trust.
...
ArgoCD automatically applies resources defined in Git similar to how this guide uses `kubectl apply`.
...
argocd-notifications-controller-67c856dbb7-rxstk    1/1     Running     0          10s
...
Apply the AuthPolicy to secure ArgoCD with Zero Trust.

```bash
kubectl apply --server-side=true -f ./deploy/clusters/workload/components/argo-authpolicy
```

Apply the HTTPRoute to expose ArgoCD through the default Gateway.
...
automic reconciliation of platform components.
...
![ArogCD Out of Sync](./argocd-out-of-sync.png)

Review the differences between the live platform and the git configuration.

![ArogCD Diff](./argocd-diff.png)

Sync the application to reconcile the differences.

![ArogCD Sync](./argocd-sync.png)

The Holos olos component should report Sync OK.

![ArogCD Sync OK](./argocd-sync-ok.png)
...
CUE merges definitions located in multiple files.  This feature is used to customize the platform.
