...
HelmChart: #Helm & {
	Name: "podinfo"
	Chart: {
		// version is a string with a default value of 6.6.1
		// @tag indicates values may be injected from the platform spec.  We call
		// these injected values Component Parameters.
		version: string | *"6.6.1" @tag(podinfo_version, type=string)
		repository: {
			name: "podinfo"
...
Platform: Components: podinfo: {
	...
	// Inject version 6.6.2 to override the component default value.
	parameters: podinfo_version: "6.6.2"
}
```
  </TabItem>
</Tabs>
...
For example, we could change the message being displayed by passing the
`ui.message` value to the chart.

```shell
touch components/podinfo/values.cue
```
```cue
package holos
...
Holos repeats this process for every Component added to the Platform, and since
`podinfo` is the only Component all we need to do is render the platform again
to see the greeting.
...
Now the manifest will have the UI message:

```shell
grep -C2 Hello deploy/components/podinfo/podinfo.gen.yaml
```
```yaml
        env:
        - name: PODINFO_UI_MESSAGE
          value: Hello Holos from Podinfo!
        - name: PODINFO_UI_COLOR
          value: '#34577c'
```
