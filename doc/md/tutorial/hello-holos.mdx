...
Render a manifest for `podinfo` using the `holos render platform`
command.
...
caching `podinfo` locally. The `platform/` directory is the main entrypoint for
this command, but you can specify a different directory using the `--platform`
option.
:::
...
We run `holos render platform` because the CUE files in the platform
