...
| `platform/podinfo.cue` | Integrates the `podinfo` Helm component into the platform. |
| `platform/platform.gen.cue` | Exports the [Platform] spec from CUE to `holos` for processing. |
| `cue.mod` | [CUE Module] directory containing schema definitions for Kubernetes resources. |
| `resources.cue` | Defines the `#Resources` schema of common Kubernetes resources. |
| `schema.cue` | Configures the common component kinds in a consistent way by composing the `#ComponentConfig` schema definition in to each kind. |
| `tags.cue` | Holds parameter values passed from `holos render platform` to `holos render component` injected via [CUE Tags]. |
