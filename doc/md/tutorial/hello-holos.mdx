...
By default, Holos looks for configuration in `platform/`. Override with
`--platform <dir>`.
...
export a [Platform] resource to `holos`. The `./platform` directory is the entry
point to the platform rendering process.
