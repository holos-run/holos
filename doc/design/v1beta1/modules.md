...
later phases as `modules.md#m1-publishing` and so on.  The layer model and the transparency principle this chapter
builds on are fixed in the [README](README.md#the-layer-model-platform--profile--role--component);
the TaskSet a component module produces is fixed in [schema.md](schema.md).
...
- **The remote-pull chart cache.**  The v1alpha6 Helm generator pulls charts
  from a remote repository into a per-component cache.  From
  [`internal/component/v1alpha6/v1alpha6.go`](../../../internal/component/v1alpha6/v1alpha6.go)
  (lines 151–160): `cacheDir` is `vendor/{version}` under the component
  directory, and when the `cachePath` does not exist the chart is fetched
  with `helm.PullChart` using the repository URL and credentials from the
  component's `Chart.Repository` config.  Every render on a clean checkout
  reaches the network, and the chart bytes are whatever the repository
  serves that day — the render is not hermetic.
- **The BuildContext temp-directory wiring.**  `BuildContext`
  ...
  `HolosExecutable` into the component's CUE evaluation via the
  `holos_build_context` tag just before evaluating the component.  Tasks
  interpolate these fields into command arguments and file paths.  v1beta1
  carries this mechanism over unchanged
...
A holos component package is one CUE module laid out as follows.  The tree
is normative; the design adapts holos-run/holos-paas
...
3. Export a **component function** from `#Config` to a `core.#TaskSet`
   ([schema.md](schema.md)) — the `#Component` shape fixed in the
...
5. Carry at least one `examples/<name>/` directory holding a reference
   `#Config` instance and its committed golden render; the examples are the
   package's test suite and the input to the reverse-dependency gate
...
Degenerate packages are valid: a schema-only module (vendored CRD types), a
mixin-only module (dashboards, alert thresholds, policy constraint sets)
exporting definitions that unify over other packages' output
([resources.md V6](resources.md#v6-unification-by-downstream-modules)), or
a full component package.  Requirements 2–3 apply to component packages;
requirements 1, 4, 5, and 6 apply to every package kind.
...
zip and pushes it as an OCI artifact: manifest artifactType
`application/vnd.cue.module.v1+json`, layer 0 the module zip, layer 1 a
standalone copy of `cue.mod/module.cue` so resolvers walk the dependency
graph without downloading module content.  That second layer is what makes
the reverse-dependency graph of [M7](#m7-reverse-dependency-checking) cheap
to compute.
...
- **The zip carries all module files, not only CUE files.**  With
  ...
  they are ordinary committed files.  The corollary is a lint-gate check:
  an untracked or gitignored file referenced by a task silently stays out
  of the published zip, so the package lint gate MUST verify every file a
  task references resolves inside the module and is tracked by the VCS.
- **Versions are immutable.**  A published `module@version` never changes;
  ...
  registry-tag conventions layered above immutable versions, not mutations
  of them.
...
`load.Instances` with a `load.Config` — and the loader delegates registry
...
`holos render`, including auth via the standard OCI credential helpers.
The one holos-specific registry surface is the vendor path
([M4](#m4-vendoring)), which substitutes a local directory for the network
without changing resolution semantics.

Rationale: the resolver is where module identity, security, and caching
meet; a bespoke resolver would fork all three.  The `cue` CLI and holos
MUST agree about which bytes a module path names.
...
minimum set of versions satisfying all requirements, computed
deterministically with no lock file.  The platform repository is the main
module of every render, so its `deps` entries are the floor every
resolution starts from.  A site operator pins the vault package like so:
...
keeps the deps map consistent with the imports.  Two properties follow
from MVS, stated here so operators are not surprised:

- **A pin is a floor, not a ceiling.**  If another dependency requires
  `vault@v0.4.0`, MVS selects v0.4.0.  The selection is still
  deterministic and still visible: it is a function of the committed deps
  closure, never of time or registry state.
- **The exact build list is recorded at vendor time.**  MVS needs no lock
  file for determinism, but operators want an auditable record of what a
  render used; the vendor manifest of [M4](#m4-vendoring) is that record,
  and CI SHOULD fail when the manifest disagrees with re-resolution — the
  same diff-clean discipline the golden renders use.
...
(the pinned minor/patch).  Putting pins anywhere else would scatter site
truth into reusable layers.
...
- **Idempotency**: vendoring is content-addressed.  A directory whose
  ...
  longer in the build list are removed.  Running `holos module vendor`
  twice is a no-op, and the second run needs no network when the first
  succeeded.
- **Offline renders**: when `vendor/modules/` exists, `holos render`
  resolves module imports from the vendor tree — the loader is configured
  with a registry view backed by the vendor directory instead of the
  network — and task file access ([M6](#m6-module-file-access-at-render-time))
  reads module files from the same tree.  A vendored platform renders
  fully offline; CI SHOULD render with the network disabled to keep the
  property honest.
- **`holos module pull`** is the single-module form: fetch, verify, and
  ...
  manifest.  It exists for inspection and for incremental updates; vendor
  is pull over the whole build list plus removal of stale entries.

Committing `vendor/modules/` is the site operator's choice.  Committing
trades repository size for a fully hermetic repo (clone-and-render with no
registry); not committing keeps the repo small and relies on the manifest
digests to make CI vendoring reproducible.  Both are supported; the
manifest is committed either way.
...
CUE's `@embed()` attribute cannot embed binary files — the embed
interpreter (already enabled by holos:
`cuecontext.Interpreter(embed.New())` in
[`internal/component/cue.go`](../../../internal/component/cue.go)) loads
text and structured formats into CUE values — but the chart archive never
needs to enter CUE evaluation at all.  Tasks read files from the module
directory on the filesystem at render time ([M6](#m6-module-file-access-at-render-time));
`helm template` consumes the `.tgz` path directly.
...
- When `source` is set, the executor resolves it per
  ...
  against the resolved archive path.  No pull occurs, no cache directory
  is consulted, and the platform-wide vendor-task deduplication of
...
- When `repository` is set instead, the remote-pull path carries over:
  ...
  cache.  This keeps migration incremental — existing components keep
  rendering — but the packaging convention for published modules is the
  embedded chart, because a module whose render reaches a chart
  repository is not hermetic and fails package lint.
...
made the *cache* live in the consumer's tree and the *bytes* come from the
network.  M5 inverts both: the bytes live in the publisher's module,
versioned and content-addressed with it, and upgrading a chart is a
reviewable module release whose golden-render diff shows exactly what
changed.
...
1. **Locate the module root.**  An empty `module` resolves to the platform
   ...
   ([M4](#m4-vendoring)), where `<version>` is the build-list selection of
   [M3](#m3-version-selection-and-platform-pinning); when the platform is
   not vendored, it resolves to the standard CUE module cache entry for
   the same selection.  A module absent from the build list is an error
   naming the task and the module path — a task cannot read from a module
...
2. **Join and contain.**  The executor joins the module root with `path`,
   ...
   resolved (`filepath.EvalSymlinks`) and the real path is re-verified
...
The rule exists because modules are third-party inputs: a platform that
vendors a community module must be able to trust that the module's tasks
read the module and nothing else.  Combined with the transparency
principle (no environment reads, no cluster reads), the reachable inputs
of a render are exactly: the platform repo, the vendored modules, and the
values holos injects.
...
keep sharing the per-TaskSet temp directory for intermediate files, and
`Command` args keep interpolating `buildContext` fields.  `ModuleFileRef`
is deliberately not a BuildContext field: BuildContext answers "where is
this render happening", which is one question per TaskSet, while module
file resolution answers "where is this dependency's content", which is one
question per reference and depends on the version selection — so it lives
in the executor, keyed by the build list, not in the injected tag.  Phase
3 extends the carried-over config structs that accept file paths today
(the `File` task source, Kustomize refs) to accept a `ModuleFileRef` in
addition to a component-relative path, without changing the task I/O
contract of [schema.md](schema.md#task-kinds).
...
pattern: renders are hermetic ([M5](#m5-embedded-helm-charts),
[M6](#m6-module-file-access-at-render-time)), parallelizable, and need no
cluster.  The design adapts holos-paas
`docs/research/cue-module-distribution.md` §4.8 into holos commands.

**The dependency graph.**  Module dependencies are declared in each
module's `cue.mod/module.cue` `deps` map, and every published module
exposes that file as OCI layer 1 ([M1](#m1-publishing)), so graph
...
and a promotion pipeline widens it with `--universe` (a file listing
module paths, or a registry prefix to enumerate).  Within the universe:
...
1. **Enumerate reverse dependents** of `<module>` in the universe, per
   `rdeps`.
2. **Substitute the candidate**: for each reverse dependent, construct its
   build list with the candidate version replacing the baseline (the
   currently selected version unless `--baseline` names another).
3. **Schema pass (fast, no render)**: re-evaluate each reverse
   ...
   unification or vet failure is a **schema break**; the CUE error names
   both sources, and no render is attempted for that dependent.
4. **Behavior pass (slower)**: for reverse dependents that pass the
   schema pass, re-render every `examples/<name>/` at the candidate
   version and diff against the committed golden `deploy/` tree.  Any
   diff is a **behavior break** — not necessarily wrong, but never
   silent: an intentional behavior change ships with regenerated golden
   renders in the dependents, or with a major-version bump.
...
blocking.  A promotion pipeline uses the report directly: publish to an
`unstable` channel freely after package lint; promote to `testing` when
`holos module check` passes over the distribution's universe; `stable`
adds human release management.  Channels are registry conventions above
immutable versions ([M1](#m1-publishing)); holos owns the check, not the
channel bookkeeping.
...
The pin lands in the platform's `cue.mod/module.cue` deps map
([M3](#m3-version-selection-and-platform-pinning)); the profile layer
instantiates the component function with site `#Config` values (the CUE
snippets in M3); `holos module vendor` materializes the module — chart
archive included — under `vendor/modules/`
([M4](#m4-vendoring)); and the render resolves the module import and the
chart's `ModuleFileRef` from the vendor tree with no network
([M5](#m5-embedded-helm-charts), [M6](#m6-module-file-access-at-render-time)).
When v0.4.0 is published, the operator (or the distribution's promotion
...
- [schema.md](schema.md) fixes the TaskSet a component function produces
  and the BuildContext carry-over M6 preserves.
- [rendering.md](rendering.md) step 3 deduplicates remote chart pulls;
  M5's embedded charts are the no-pull case it already anticipates.
- [resources.md](resources.md) V6 gives mixin modules their unification
  surface over rendered output.
- [use-case.md](use-case.md) composes published modules into the
  canonical management-cluster profile.
