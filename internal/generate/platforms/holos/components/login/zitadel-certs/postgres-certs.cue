...
let Objects = {
	...
	Resources: {
		// Put everything in the zitadel namespace.
		[_]: {
			[NAME=_]: {
				metadata: name:      NAME
				metadata: namespace: _ZitadelNamespace
			}
		}
		...
		Certificate: {
			...
			"\(_DBName)-primary-tls": #DatabaseCert & {
				...
				spec: {
					...
					dnsNames: [
						commonName,
						"\(commonName).\(_ZitadelNamespace)",
						"\(commonName).\(_ZitadelNamespace).svc",
						"\(commonName).\(_ZitadelNamespace).svc.cluster.local",
						"localhost",
						"127.0.0.1",
					]
