...
#PrometheusSpec: {
	...
	// containing additional Prometheus Alertmanager configurations.
	// The Alertmanager configurations are appended to the
	// configuration generated by the Prometheus Operator. They must
	// be formatted according to the official Prometheus
	// documentation:
	...
	// Prometheus release notes to ensure that no incompatible
	...
	// containing additional Prometheus alert relabel configurations.
	// The alert relabel configurations are appended to the
	// configuration generated by the Prometheus Operator. They must
	// be formatted according to the official Prometheus
	// documentation:
	...
	// Note that using this feature may expose the possibility to
	// break upgrades of Prometheus. It is advised to review
	// Prometheus release notes to ensure that no incompatible alert
	// relabel configs are going to break Prometheus after the
	// upgrade.
	...
	// delete data, shutdown Prometheus, and more. Enabling this
	// should be done with care and the user is advised to add
	// additional authentication authorization via a proxy to ensure
	...
	exemplars?: {
		...
		// or less than zero disables the storage.
		maxSize?: int
	}
	...
	// namespace label enforcement doesn't apply. This is only
	// relevant when `spec.enforcedNamespaceLabel` is set to true.
	...
	query?: {
		...
		maxConcurrency?: >=1 & int

		// Maximum number of samples a single query can load into memory.
		// Note that queries will fail if they would load more samples
	...
	// '/var/log/prometheus/query.log', you must mount a volume in
	// the specified directory and it must be writable. This is
	// because the prometheus container runs with a read-only root
	// filesystem for security reasons. Alternatively, the location
	// can be set to a standard I/O stream, e.g. `/dev/stdout`, to
	// log query information to the default Prometheus log stream.
	queryLogFile?: string
	...
	remoteRead?: [...{
		...
		// differentiate read configurations.
	...
	// This section is experimental, it may change significantly
	// without deprecation notice in any release.
	thanos?: {
		// AdditionalArgs allows setting additional arguments for the
		// Thanos container. The arguments are passed as-is to the Thanos
		// container which may cause issues if they are invalid or not
		// supported the given Thanos version. In case of an argument
		// conflict (e.g. an argument which is already set by the
		...
		// Prometheus. The default value is 2h to match the upstream
		// Prometheus defaults.
		// WARNING: Changing the block duration can impact the performance
		// and efficiency of the entire Prometheus/Thanos stack due to
		// how it interacts with memory and Thanos compactors. It is
		// recommended to keep this value set to a multiple of 120 times
		...
		// instead of the Pod IP's address for the gRPC endpoints.
		...
		// When true, the Thanos sidecar listens on the loopback interface
		// instead of the Pod IP's address for the HTTP endpoints.
		// It has no effect if `listenLocal` is true.
		httpListenLocal?: bool

		// Container image name for Thanos. If specified, it takes
		// precedence over the `spec.thanos.baseImage`, `spec.thanos.tag`
		...
		// Thanos available at the time when the operator was released.
		image?: string
		...
		// sidecar's StoreAPI. The field's value should be a constant
		// time in RFC3339 format or a time duration relative to current
		// time, such as -1d or 2h45m. Valid duration units are ms, s, m,
		// h, d, w, y.
		minTime?: string
		...
		// release of Thanos available at the time when the version of
	...
	tsdb?: {
		...
		// with respect to the TSDB max time.
		// An out-of-order/out-of-bounds sample is ingested into the TSDB
		// as long as the timestamp of the sample is >= (TSDB.MaxTime -
