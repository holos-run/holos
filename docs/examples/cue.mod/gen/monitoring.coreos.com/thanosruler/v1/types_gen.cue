...
#ThanosRuler: {
	...
	// Specification of the desired behavior of the ThanosRuler
	// cluster. More info:
...
#ThanosRulerSpec: {
	...
	// ThanosRuler container. It is intended for e.g. activating
	// hidden flags which are not supported by the dedicated
	// configuration options yet. The arguments are passed as-is to
	// the ThanosRuler container which may cause issues if they are
	// invalid or not supported by the given ThanosRuler version. In
	// case of an argument conflict (e.g. an argument which is
	...
	// AlertDropLabels configure the label names which should be
	// dropped in ThanosRuler alerts. The replica label
	// `thanos_ruler_replica` will always be dropped in alerts.
	alertDropLabels?: [...string]

	// The external Query URL the Thanos Ruler will set in the
	// 'Source' field of all alerts. Maps to the '--alert.query-url'
	// CLI arg.
	alertQueryUrl?: string

	// AlertRelabelConfigFile specifies the path of the alert
	// relabeling configuration file. When used alongside with
	// AlertRelabelConfigs, alertRelabelConfigFile takes precedence.
	alertRelabelConfigFile?: string

	// AlertRelabelConfigs configures alert relabeling in ThanosRuler.
	// Alert relabel configurations must have the form as specified
	// in the official Prometheus documentation:
	// https://prometheus.io/docs/prometheus/latest/configuration/configuration/#alert_relabel_configs
	// Alternative to AlertRelabelConfigFile, and lower order
	// priority.
	alertRelabelConfigs?: {
	...
	// Define configuration for connecting to alertmanager. Only
	// available with thanos v0.10.0 and higher. Maps to the
	// `alertmanagers.config` arg.
	alertmanagersConfig?: {
		// The key of the secret to select from. Must be a valid secret
		// key.
		key: string
	...
	// Define URLs to send alerts to Alertmanager. For Thanos v0.10.0
	// and higher, AlertManagersConfig should be used instead. Note:
	// this field will be ignored if AlertManagersConfig is
	// specified. Maps to the `alertmanagers.url` arg.
	...
	// adding an authentication proxy to a ThanosRuler pod or to
	// change the behavior of an operator generated container.
	// Containers described here modify an operator generated
	// container if they share the same name and modifications are
	// done via a strategic merge patch. The current container names
	// are: `thanos-ruler` and `config-reloader`. Overriding
	...
	// EnforcedNamespaceLabel enforces adding a namespace label of
	// origin for each alert and metric that is user created. The
	// label value will always be the namespace of the object that is
	// being created.
	...
	// List of references to PrometheusRule objects to be excluded
	// from enforcing a namespace label of origin. Applies only if
	// enforcedNamespaceLabel set to true.
	...
	// The external URL the Thanos Ruler instances will be available
	// under. This is necessary to generate correct URLs. This is
	// necessary if Thanos Ruler is not served from root of a DNS
	// name.
	externalPrefix?: string

	// GRPCServerTLSConfig configures the gRPC server from which
	// Thanos Querier reads recorded rule data. Note: Currently only
	// the CAFile, CertFile, and KeyFile fields are supported. Maps
	...
	// to use for pulling thanos images from registries see
	...
	// injection into the ThanosRuler configuration from external
	// sources. Any errors during the execution of an initContainer
	// will lead to a restart of the Pod. More info:
	// https://kubernetes.io/docs/concepts/workloads/pods/init-containers/
	// Using initContainers for any use case other then secret
	// fetching is entirely outside the scope of what the maintainers
	// will support and by doing so, you accept that this behaviour
	...
	// Labels configure the external label pairs to ThanosRuler. A
	// default replica label `thanos_ruler_replica` will be always
	// added as a label with the value of the pod's name and it will
	// be dropped in the alerts.
	labels?: {
		[string]: string
	}

	// ListenLocal makes the Thanos ruler listen on loopback, so that
	// it does not bind against the Pod IP.
	listenLocal?: bool
	...
	// Alternative to ObjectStorageConfigFile, and lower order
	// priority.
	objectStorageConfig?: {
		// The key of the secret to select from. Must be a valid secret
		// key.
	...
	// ObjectStorageConfigFile specifies the path of the object
	// storage configuration file. When used alongside with
	// ObjectStorageConfig, ObjectStorageConfigFile takes precedence.
	objectStorageConfigFile?: string

	// When a ThanosRuler deployment is paused, no actions except for
	// deletion will be performed on the underlying objects.
	paused?: bool

	// PodMetadata configures labels and annotations which are
	// propagated to the ThanosRuler pods.
	// The following items are reserved and cannot be overridden: *
	// "app.kubernetes.io/name" label, set to "thanos-ruler". *
	// "app.kubernetes.io/managed-by" label, set to
	// "prometheus-operator". * "app.kubernetes.io/instance" label,
	// set to the name of the ThanosRuler instance. * "thanos-ruler"
	// label, set to the name of the ThanosRuler instance. *
	...
	// PrometheusRulesExcludedFromEnforce - list of Prometheus rules
	// to be excluded from enforcing of adding namespace labels.
	// Works only if enforcedNamespaceLabel set to true. Make sure
	// both ruleNamespace and ruleName are set for each pair
	...
	// Maps to the `query.config` CLI argument. Only available with
	// thanos v0.11.0 and higher.
	queryConfig?: {
		// The key of the secret to select from. Must be a valid secret
		// key.
		key: string
	...
	// QueryEndpoints defines Thanos querier endpoints from which to
	// query metrics. Maps to the --query flag of thanos ruler.
	queryEndpoints?: [...string]

	// Number of thanos ruler instances to deploy.
	replicas?: int

	// Resources defines the resource requirements for single Pods. If
	// not provided, no requests/limits will be set
	resources?: {
		// Claims lists the names of resources, defined in
		// spec.resourceClaims, that are used by this container.
		// This is an alpha field and requires enabling the
	...
	// Time duration ThanosRuler shall retain data for. Default is
	// '24h', and must match the regular expression
	// `[0-9]+(ms|s|m|h|d|w|y)` (milliseconds seconds minutes hours
	// days weeks years).
	retention?: =~"^(0|(([0-9]+)y)?(([0-9]+)w)?(([0-9]+)d)?(([0-9]+)h)?(([0-9]+)m)?(([0-9]+)s)?(([0-9]+)ms)?)$" | *"24h"

	// The route prefix ThanosRuler registers HTTP handlers for. This
	// allows thanos UI to be served on a sub-path.
	routePrefix?: string

	// Namespaces to be selected for Rules discovery. If unspecified,
	// only the same namespace as the ThanosRuler object is in is
	// used.
	...
	// A label selector to select which PrometheusRules to mount for
	...
	// TracingConfig configures tracing in Thanos. This is an
	// experimental feature, it may change in any upcoming release in
	// a breaking way.
	tracingConfig?: {
		// The key of the secret to select from. Must be a valid secret
		// key.
	...
	// TracingConfig specifies the path of the tracing configuration
	// file. When used alongside with TracingConfig,
	...
	// be appended to other VolumeMounts in the ruler container, that
