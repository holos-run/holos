...
#Alertmanager: {
	...
	// Specification of the desired behavior of the Alertmanager
	// cluster. More info:
...
// https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
#AlertmanagerSpec: {
	// AdditionalPeers allows injecting a set of additional
	...
	alertmanagerConfigMatcherStrategy?: {
		// If set to `OnNamespace`, the operator injects a label matcher
		// matching the namespace of the AlertmanagerConfig object for
		// all its routes and inhibition rules. `None` will not add any
		// additional matchers other than the ones specified in the
		// AlertmanagerConfig. Default is `OnNamespace`.
		type?: "OnNamespace" | "None" | *"OnNamespace"
	}

	// Namespaces to be selected for AlertmanagerConfig discovery. If
	...
	// AlertmanagerConfigs to be selected for to merge and configure
	// Alertmanager with.
	...
	// EXPERIMENTAL: alertmanagerConfiguration specifies the
	// configuration of Alertmanager. If defined, it takes precedence
	// over the `configSecret` field. This field may change in future
	// releases.
	alertmanagerConfiguration?: {
		// Defines the global parameters of the Alertmanager
		// configuration.
		global?: {
			...
			httpConfig?: {
				...
				// same namespace as the Alertmanager object and accessible by
			...
			// ResolveTimeout is the default value used by alertmanager if the
			// alert does not include EndsAt, after this time passes it can
			// declare the alert as resolved if it has not been updated. This
			// has no impact on alerts from Prometheus, as they always
			// include EndsAt.
			...
			smtp?: {
				...
				// SMTP Auth using CRAM-MD5, LOGIN and PLAIN. If empty,
				...
				// The default SMTP TLS requirement. Note that Go does not support
				// unencrypted connections to remote SMTP endpoints.
				requireTLS?: bool

				// The default SMTP smarthost used for sending emails.
				smartHost?: {
					// Defines the host's address, it can be a DNS name or a literal
					// IP address.
					host: strings.MinRunes(1)

					// Defines the host's port, it can be a literal port number or a
					// port name.
		...
		// The name of the AlertmanagerConfig resource which is used to
		// generate the Alertmanager configuration. It must be defined in
		// the same namespace as the Alertmanager object. The operator
	...
	// account token should be automatically mounted in the pod. If
	// the service account has `automountServiceAccountToken: true`,
	// set the field to `false` to opt out of automounting API
	// credentials.
	...
	// ClusterAdvertiseAddress is the explicit address to advertise in
	// cluster. Needs to be provided for non RFC1918 [1] (public)
	// addresses. [1] RFC1918: https://tools.ietf.org/html/rfc1918
	clusterAdvertiseAddress?: string

	// Interval between gossip attempts.
	clusterGossipInterval?: =~"^(0|(([0-9]+)h)?(([0-9]+)m)?(([0-9]+)s)?(([0-9]+)ms)?)$"

	// Defines the identifier that uniquely identifies the
	// Alertmanager cluster. You should only set it when the
	// Alertmanager cluster includes Alertmanager instances which are
	// external to this Alertmanager resource. In practice, the
	...
	// Alertmanager object, which shall be mounted into the
	// Alertmanager Pods. Each ConfigMap is added to the StatefulSet
	// definition as a volume named `configmap-<configmap-name>`. The
	// ConfigMaps are mounted into
	// `/etc/alertmanager/configmaps/<configmap-name>` in the
	// 'alertmanager' container.
	configMaps?: [...string]

	// ConfigSecret is the name of a Kubernetes Secret in the same
	// namespace as the Alertmanager object, which contains the
	// configuration for this Alertmanager instance. If empty, it
	// defaults to `alertmanager-<alertmanager-name>`.
	// The Alertmanager configuration should be available under the
	// `alertmanager.yaml` key. Additional keys from the original
	// secret are copied to the generated secret and mounted into the
	// `/etc/alertmanager/config` directory in the `alertmanager`
	// container.
	// If either the secret or the `alertmanager.yaml` key is missing,
	// the operator provisions a minimal Alertmanager configuration
	// with one empty receiver (effectively dropping alert
	// notifications).
	configSecret?: string

	// Containers allows injecting additional containers. This is
	// meant to allow adding an authentication proxy to an
	// Alertmanager pod. Containers described here modify an operator
	// generated container if they share the same name and
	// modifications are done via a strategic merge patch. The
	// current container names are: `alertmanager` and
	// `config-reloader`. Overriding containers is entirely outside
	// the scope of what the maintainers will support and by doing
	// so, you accept that this behaviour may break at any time
	// without notice.
	containers?: [...{
	...
	// The external URL the Alertmanager instances will be available
	// under. This is necessary to generate correct URLs. This is
	// necessary if Alertmanager is not served from root of a DNS
	// name.
	externalUrl?: string

	// ForceEnableClusterMode ensures Alertmanager does not deactivate
	// the cluster mode when running with a single replica. Use case
	// is e.g. spanning an Alertmanager cluster across Kubernetes
	...
	// combinations. Specifying the version is still necessary to
	// ensure the Prometheus Operator knows what version of
	// Alertmanager is being configured.
	image?: string

	// Image pull policy for the 'alertmanager',
	// 'init-config-reloader' and 'config-reloader' containers. See
	// https://kubernetes.io/docs/concepts/containers/images/#image-pull-policy
	// for more details.
	imagePullPolicy?: "" | "Always" | "Never" | "IfNotPresent"

	// An optional list of references to secrets in the same namespace
	// to use for pulling prometheus and alertmanager images from
	// registries see
	...
	// InitContainers allows adding initContainers to the pod
	// definition. Those can be used to e.g. fetch secrets for
	// injection into the Alertmanager configuration from external
	...
	// are done via a strategic merge patch. The current init
	// container name is: `init-config-reloader`. Overriding init
	// containers is entirely outside the scope of what the
	...
	// ListenLocal makes the Alertmanager server listen on loopback,
	// so that it does not bind against the Pod IP. Note this is only
	// for the Alertmanager UI, not the gossip communication.
	listenLocal?: bool
	...
	// If set to true all actions on the underlying managed objects
	...
	// "alertmanager" label, set to the name of the Alertmanager
	// instance. * "app.kubernetes.io/instance" label, set to the
	// name of the Alertmanager instance. *
	// "app.kubernetes.io/managed-by" label, set to
	// "prometheus-operator". * "app.kubernetes.io/name" label, set
	// to "alertmanager". * "app.kubernetes.io/version" label, set to
	// the Alertmanager version. *
	...
	// Size is the expected size of the alertmanager cluster. The
	// controller will eventually make the size of the running
	...
	// Time duration Alertmanager shall retain data for. Default is
	// '120h', and must match the regular expression
	// `[0-9]+(ms|s|m|h)` (milliseconds seconds minutes hours).
	retention?: =~"^(0|(([0-9]+)h)?(([0-9]+)m)?(([0-9]+)s)?(([0-9]+)ms)?)$" | *"120h"

	// The route prefix Alertmanager registers HTTP handlers for. This
	// is useful, if using ExternalURL and a proxy is rewriting HTTP
	// routes of a request, and the actual ExternalURL is still true,
	// but the server serves requests under a different route prefix.
	// For example for use with `kubectl proxy`.
	routePrefix?: string

	// Secrets is a list of Secrets in the same namespace as the
	// Alertmanager object, which shall be mounted into the
	// Alertmanager Pods. Each Secret is added to the StatefulSet
	// definition as a volume named `secret-<secret-name>`. The
	// Secrets are mounted into
	// `/etc/alertmanager/secrets/<secret-name>` in the
	// 'alertmanager' container.
	secrets?: [...string]
	...
	// SHA of Alertmanager container image to be deployed. Defaults to
	// the value of `version`. Similar to a tag, but the SHA
	// explicitly deploys an immutable container image. Version and
	// Tag are ignored if SHA is set. Deprecated: use 'image'
	...
	// Tag of Alertmanager container image to be deployed. Defaults to
	// the value of `version`. Version is ignored if Tag is set.
	// Deprecated: use 'image' instead. The image tag can be
	...
	// VolumeMounts allows configuration of additional VolumeMounts on
	// the output StatefulSet definition. VolumeMounts specified will
	// be appended to other VolumeMounts in the alertmanager
	...
	// Volumes allows configuration of additional volumes on the
	...
	web?: {
		// Maximum number of GET requests processed concurrently. This
		// corresponds to the Alertmanager's `--web.get-concurrency`
		// flag.
		getConcurrency?: int
		...
		// Timeout for HTTP requests. This corresponds to the
