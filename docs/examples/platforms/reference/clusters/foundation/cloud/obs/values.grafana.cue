...
#KubePromStackValues: {
	...
	grafana: {
		...
		"grafana.ini": {
			...
			analytics: check_for_updates: *true | false
			log: mode:                    "console"
			grafana_net: url:             string | *"https://grafana.net"
			server: domain:               string | *"{{ if (and .Values.ingress.enabled .Values.ingress.hosts) }}{{ .Values.ingress.hosts | first }}{{ else }}''{{ end }}"
